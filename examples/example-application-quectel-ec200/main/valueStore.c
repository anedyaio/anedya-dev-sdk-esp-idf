...
void valueStore_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (ulNotifiedValue == 0x01)
        {
            ...
            {
                ESP_LOGI(TAG, "--------------------------------");
                ESP_LOGI(TAG, "%s: Key:%s, Value: %s  Key Value Set", TAG, strKey, strValue);
                ESP_LOGI(TAG, "--------------------------------");
            }
        }
        else
        ...
        if (ulNotifiedValue == 0x01)
        {
            if (vs_txn.is_success && vs_txn.is_complete)
            {
                ...
                {
                    ESP_LOGI(TAG, "Namespace: scope: %s, Id: %s", resp_obj_list.keys[i].ns.scope, resp_obj_list.keys[i].ns.id);
                    ESP_LOGI(TAG, "Key :%s, Type: %s, Modified: %lld", resp_obj_list.keys[i].key, resp_obj_list.keys[i].type, resp_obj_list.keys[i].modified); 
                }
                ESP_LOGI(TAG, "-------------------------------------------------------------");
            }
            else
