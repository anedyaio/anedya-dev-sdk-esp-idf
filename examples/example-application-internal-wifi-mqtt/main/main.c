...
void cl_event_handler(anedya_client_t *client, anedya_event_t event,
  ...
  // ============================================== Valuestore Handler
  // =====================================================
  //                                      It Will be called when valuestore is
  //                                      updated
  ...
  case ANEDYA_EVENT_VS_UPDATE_BOOL:
    ...
    ESP_LOGI("CLIENT", "Key Updated: %s Value:%d", data_bool->key,
             data_bool->value);
    break;
  case ANEDYA_EVENT_VS_UPDATE_STRING:
    ...
    ESP_LOGI("CLIENT", "Key Updated: %s Value:%s", data_str->key,
             data_str->value);
    break;
  case ANEDYA_EVENT_VS_UPDATE_BIN:
    ...
    ESP_LOGI("CLIENT", "Key Updated: %s Value:%s", data_bin->key,
             data_bin->value);
    break;
...
void app_main(void) {
  char connkey[64] =
      CONFIG_CONNECTION_KEY; // Connection Key to connect to anedya
  anedya_device_id_t devid;
  anedya_parse_device_id(CONFIG_PHYSICAL_DEVICE_ID,
                         devid); // UUID of the device
  ...
  // =============== Anedya Config  // ===========================
  anedya_config_init(&anedya_client_config, devid, connkey,
                     strlen(connkey)); // Initialize config
  anedya_config_set_connect_cb(&anedya_client_config, MQTT_ON_Connect,
                               &event_group); // Set connect callback
  anedya_config_set_disconnect_cb(&anedya_client_config, MQTT_ON_Disconnect,
                                  NULL); // Set disconnect callback
  anedya_config_register_event_handler(&anedya_client_config, cl_event_handler,
                                       NULL); // Set event handler
  anedya_config_set_region(&anedya_client_config,
                           ANEDYA_REGION_AP_IN_1);         // Set region
...
// ================================================================
  ...
  xTaskCreate(ota_management_task, "OTA", 20 * 1024, &gatewaystate, 1, NULL);
  // Start OTA Task
  xTaskCreate(submitData_task, "SUBMITDATA", 6 * 1024, NULL, 2,
              NULL); // Start Submit Data Task
  xTaskCreate(valueStore_task, "VALUESTORE", 10240, NULL, 4,
              NULL); // Start Valuestore Task
  xTaskCreate(commandHandling_task, "COMMANDHANDLER", 12 * 1024, NULL, 1,
              NULL); // Start Command Handler
  xTaskCreate(submitLog_task, "SUBMITLOG", 6 * 1024, NULL, 4,
              NULL); // Start Submit Log

  for (;;) {
    // ================================================ Send Heartbeat to Anedya
    // ================================================ For more info visit:
    // https://docs.anedya.io/getting-started/quickstart/

    xEventGroupWaitBits(ConnectionEvents, MQTT_CONNECTED_BIT, pdFALSE, pdFALSE,
                        portMAX_DELAY);
