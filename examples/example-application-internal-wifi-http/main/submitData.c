...
static bool submitFloatData(char *variable_identifier, float variable_value,
                            uint64_t timestamp_ms);
static bool submitGeoData(char *variable_identifier, float latitude,
                          float longitude, uint64_t timestamp_ms);

void submitData_task(void *pvParameters) {
  /* Wait for network connectivity regardless of transport (WiFi or LTE) */
...
static bool submitFloatData(char *variable_identifier, float variable_value,
  ...
  // Submit Data
  anedya_err_t aerr =
      anedya_op_submit_float_req(&anedya_client, &sd_txn, variable_identifier,
                                 variable_value, timestamp_ms);
  if (aerr != ANEDYA_OK) {
  ...
  if (ulNotifiedValue == 0x01) {
    if (sd_txn.is_success && sd_txn.is_complete) {
      ...
      ESP_LOGE(TAG,
               "Failed to pushed data to Anedya, Check Variable Identifier");
      return false;
    }
  } else {
...
static bool submitGeoData(char *variable_identifier, float latitude,
  ...
  // Submit Data
  anedya_err_t aerr = anedya_op_submit_geo_req(
      &anedya_client, &sd_txn, variable_identifier, &geo_data, timestamp_ms);
  if (aerr != ANEDYA_OK) {
    ESP_LOGI("CLIENT", "%s", anedya_err_to_name(aerr));
  }
  xTaskNotifyWait(0x00, ULONG_MAX, &ulNotifiedValue,
  ...
  if (ulNotifiedValue == 0x01) {
    if (sd_txn.is_success && sd_txn.is_complete) {
      ...
      ESP_LOGE(
          TAG,
          "Failed to pushed Geo data to Anedya, Check Variable Identifier");
      return false;
    }
  } else {
    ESP_LOGI("CLIENT", "TXN Timeout");
