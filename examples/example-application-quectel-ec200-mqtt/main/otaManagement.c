  ...
  for (;;) {
    ...
    if (ota_txn.is_success) {
      ...
      if (resp.deployment_available) {
        ...
        ESP_LOGI(TAG, "Asset Size: %d", (int)resp.asset.asset_size);
        ...
        while (check_update_status < 5) {
          ...
          anedya_txn_register_callback(&update_txn, TXN_COMPLETE,
                                       &current_task);
          anedya_err_t uerr = anedya_op_ota_update_status_req(
              &anedya_client, &update_txn, &update_status);
          if (uerr != ANEDYA_OK) {
            ESP_LOGI(TAG, "%s", anedya_err_to_name(uerr));
          }
          xTaskNotifyWait(0x00, ULONG_MAX, &ulNotifiedValue,
                          30000 / portTICK_PERIOD_MS);
          if (ulNotifiedValue == 0x01) {
            ...
            if (update_txn.is_success) {
              ...
              anedya_txn_register_callback(&conclude_txn, TXN_COMPLETE,
                                           &current_task);
              ...
              while (check_update_status < 5) {
                ...
                anedya_err_t cerr = anedya_op_ota_update_status_req(
                    &anedya_client, &conclude_txn, &conclude_status);
                if (cerr != ANEDYA_OK) {
              ...
              if (otaerr == ESP_OK) {
                ESP_LOGI(TAG, "Firmware has been updated. Waiting for shutdown "
                              "process to complete.");
                ...
                ESP_LOGI(TAG, "Firmware update failed. Restarting device for "
                              "continued operation!");
                for (int i = 0; i < 5; i++) {
                  ESP_LOGI(TAG, "Device will restart in: %d seconds", (5 - i));
                  vTaskDelay(1000 / portTICK_PERIOD_MS);
...
static esp_err_t do_firmware_update(anedya_op_next_ota_resp_t *resp) {
  ...
  esp_err_t err =
      esp_ota_begin(ota_partition, resp->asset.asset_size, &ota_handle);
  if (err != ESP_OK) {
  ...
  while (start_position < resp->asset.asset_size) {
    ...
    anedya_err_t oerr = anedya_ext_http_get_range_request(
        &anedya_client, &reader, resp->asset.asset_url,
        resp->asset.asset_url_len, start_position, chunk_size, 10000);
    if (oerr != ANEDYA_OK)
      continue;
    vTaskDelay(500 / portTICK_PERIOD_MS);
    if (reader.content_length > 0) {
      while (reader.bytes_read != reader.content_length) {
        ...
        size_t read = anedya_ext_ota_read_next(&anedya_client, &reader,
                                               read_len, data, 2000);
        if (read > 0) {
          uint32_t offset = start_position + reader.bytes_read - read;
          err = esp_ota_write_with_offset(ota_handle, data, read,
                                          (uint32_t)offset);
          if (err != ESP_OK) {
          ...
          ESP_LOGI(TAG,
                   "Bytes read %d, Read Asset size %d, Percentage: %f %%  \n",
                   start_position + reader.bytes_read, resp->asset.asset_size,
                   ((float)(start_position + reader.bytes_read) /
                    (float)resp->asset.asset_size) *
                       100);
        }
