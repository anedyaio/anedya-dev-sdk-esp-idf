...
static uint8_t dtmp[ANEDYA_RX_BUFFER_SIZE + 256] = {
    0}; // Increased size to handle large bursts
...
static anedya_err_t
_anedya_ext_clear_uart_buffer(anedya_client_t *anedya_client);
static void _anedya_ext_mqtt_event_task(void *pvParameters);
static void _mqtt_message_parser(anedya_client_t *anedya_client,
                                 uart_event_t event);
static anedya_err_t _anedya_ext_send_AT_command(char *cmd,
...
static void _anedya_ext_mqtt_event_task(void *pvParameters) {
  ...
  while (1) {
    xEventGroupWaitBits(MqttEvents, MQTT_EVENT_RECEIVE_DATA, pdFALSE, pdFALSE,
                        portMAX_DELAY);
    xEventGroupClearBits(MqttEvents, MQTT_EVENT_RECEIVE_DATA);
    ...
    case EXT_MQTT_EVENT_DATA:
      if (anedya_client->_message_handler != NULL) {
        anedya_client->_message_handler(anedya_client, mqtt_event.topic,
                                        mqtt_event.topic_len, mqtt_event.data,
                                        mqtt_event.data_len);
      }
      continue;
    default:
...
static void _mqtt_message_parser(anedya_client_t *anedya_client,
                                 uart_event_t event) {
  // int dummy1, dummy2; // Unused after sscanf fix
  ...
  if (recv_ptr != NULL) {
    ...
    int matched = sscanf(recv_ptr, "+QMTRECV: %*d,%*d,\"%149[^\"]\",%d",
                         (char *)response_topic, &modem_response_len);
    if (matched) {
      if (modem_response_len > 9) {
        char *start_ptr = strchr((char *)dtmp, '{');
        if (start_ptr) {
          size_t available_in_dtmp =
              (uint8_t *)dtmp + sizeof(dtmp) - (uint8_t *)start_ptr - 1;
          size_t copy_len = (modem_response_len < available_in_dtmp)
                                ? modem_response_len
                                : available_in_dtmp;
          if (copy_len > ANEDYA_RX_BUFFER_SIZE - 1)
            copy_len = ANEDYA_RX_BUFFER_SIZE - 1;

          strncpy((char *)modem_response, (char *)start_ptr, copy_len);
          modem_response[copy_len] = '\0';
        }
      }
    ...
    if (modem_response_len < 10) {
      ...
      size_t current_len = strlen((char *)modem_response);
      size_t remaining_to_copy = modem_response_len - current_len;
      size_t available_in_dtmp = strlen((char *)dtmp);
      size_t copy_len = (remaining_to_copy < available_in_dtmp)
                            ? remaining_to_copy
                            : available_in_dtmp;

      if (current_len + copy_len > ANEDYA_RX_BUFFER_SIZE - 1) {
        copy_len = ANEDYA_RX_BUFFER_SIZE - 1 - current_len;
      }

      strncat((char *)modem_response, (char *)dtmp, copy_len);
...
static void _uart_event_task(void *pvParameters) {
  ...
  QueueHandle_t uart_queue =
      ((anedya_ext_config_t *)anedya_client->config->interface_config)
          ->uart_queue_handle;
  ...
  for (;;) {
    xEventGroupWaitBits(ModemEvents, MODEM_EVENT_OTA_NOT_IN_PROGRESS, pdFALSE,
    ...
    if (xQueueReceive(uart_queue, (void *)&event, (TickType_t)portMAX_DELAY)) {
      ...
      case UART_DATA:
        ...
        int len = uart_read_bytes(
            UART_PORT_NUMBER, dtmp,
            (event.size < sizeof(dtmp) - 1) ? event.size : (sizeof(dtmp) - 1),
            pdMS_TO_TICKS(2000));
        if (len <= 0)
          break;
        int j = 0;
      ...
      case UART_PATTERN_DET:
        ...
        ESP_LOGI("UART EVENT HANDLER",
                 "[UART PATTERN DETECTED] pos: %d, buffered size: %d", pos,
                 buffered_size);
        if (pos == -1) {
          uart_flush_input(UART_PORT_NUMBER);
        } else {
          uart_read_bytes(UART_PORT_NUMBER, dtmp, pos,
          ...
          uart_read_bytes(UART_PORT_NUMBER, pat, PATTERN_CHR_NUM,
                          100 / portTICK_PERIOD_MS);
  ...
  if (cmd_type == MODEM_RESP_WAIT) {
    ...
    if (expected_resp != NULL) {
      while (xTaskGetTickCount() - start_time < timeout / portTICK_PERIOD_MS) {
        ...
        // if (strncmp((char *)dtmp, expected_resp, strlen((char
        // *)expected_resp)) == 0)
        if (check_ptr != NULL) {
          // ESP_LOGI("RX Commnad:", "%s", dtmp);
          if (resp != NULL && resp_size > 0) {
            strncpy(resp, (char *)dtmp, resp_size - 1);
            resp[resp_size - 1] = '\0';
          }
      ...
      // ESP_LOGI(TAG, "Waiting for response from modem with timeout: %d ms",
      // timeout);
...
anedya_err_t _anedya_interface_init(anedya_client_t *client) {
  anedya_ext_config_t *ext_config =
      (anedya_ext_config_t *)client->config->interface_config;
  ...
  uart_driver_install(ext_config->uart_port_num, ANEDYA_RX_BUFFER_SIZE,
                      ANEDYA_TX_BUFFER_SIZE, ANEDYA_RX_BUFFER_SIZE / 120,
                      &ext_config->uart_queue_handle, 0);
  uart_param_config(ext_config->uart_port_num, &ext_config->uart_config);

  // Set UART pins
  uart_set_pin(ext_config->uart_port_num, ext_config->tx_pin,
               ext_config->rx_pin, ext_config->rts_pin,
               ext_config->cts_pin); // Mcu RTS PIN, Mcu CTS PIN

  // Enable pattern detection
  if (uart_enable_pattern_det_baud_intr(ext_config->uart_port_num, '+',
                                        PATTERN_CHR_NUM, 9, 0, 0) != ESP_OK) {
  ...
  // Create the task
  if (xTaskCreate(_uart_event_task, "uart_event_task", 4 * 1024, client, 1,
                  NULL) != pdPASS) {
  ...
  err =
      _anedya_ext_send_AT_command("ATE0\r\n", MODEM_CMD_EX_ASAP, NULL, 0, "OK",
                                  2000); // disable echo

  // Configure Flow Control
  int dce_by_dte = 0; // TE receiving from TA (Modem -> ESP)
  int dte_by_dce = 0; // TA receiving from TE (ESP -> Modem)

  switch (ext_config->uart_config.flow_ctrl) {
  ...
  if (xSemaphoreTake(uart_port_mutex, 5000 / portTICK_PERIOD_MS) == pdTRUE) {
    ...
    _anedya_ext_send_AT_command("AT+IFC?\r\n", MODEM_RESP_WAIT, NULL, 0, "OK",
                                2000);
    xSemaphoreGive(uart_port_mutex);
  }
  if (xTaskCreate(_anedya_ext_mqtt_event_task, "MQTT_EVENT_TASK", 4 * 1024,
                  client, 2, NULL) != pdPASS)
    ESP_LOGE(TAG, "Failed to create MQTT Event task");

  err =
      _anedya_ext_send_AT_command("AT+QMTDISC=0\r\n", MODEM_CMD_EX_ASAP, NULL,
                                  0, NULL, 2000); // disconnect from MQTT broker
      ...
      while (1) {
        ...
        if (err == ANEDYA_OK) {
          ...
          for (int i = 0; i < ext_config->apn_count; i++) {
            ...
            anedya_ext_set_apn(client, apn_config[i].cid, apn_config[i].ip_ver,
                               apn_config[i].apn, apn_config[i].username,
                               apn_config[i].password);
            vTaskDelay(50 / portTICK_PERIOD_MS);
          }
          err = anedya_ext_set_fun_mode(client, 0, 0, 2000);
          err = anedya_ext_set_fun_mode(client, 1, 0, 10000);
        }
        check++;
        if (check > 4)
          return ANEDYA_EXT_ERR;
...
anedya_err_t
anedya_ext_restore_settings_to_factory_defaults(anedya_client_t *client,
                                                int timeout) {
  if (UART_PORT_NUMBER == -1) {
    ESP_LOGE(TAG, "Invalid port number");
    return ANEDYA_EXT_ERR;
  ...
  // if (value < 0)
  //     return ANEDYA_EXT_ERR;
  if (xSemaphoreTake(uart_port_mutex, (timeout + 5000) / portTICK_PERIOD_MS) !=
      pdTRUE) {
    ESP_LOGE(TAG, "Failed to take uart_port_mutex!");
    return ANEDYA_ERR_EXT_TIMEOUT;
  }
#ifdef ANEDYA_ENABLE_STATIC_ALLOCATION
  char AT_cmd[20];
...
// TODO: Implement dynamic allocation
  ...
  sprintf(AT_cmd, "AT&F\r\n");
  anedya_err_t err = _anedya_ext_send_AT_command(AT_cmd, MODEM_RESP_WAIT, NULL,
                                                 0, "OK", timeout);
  xSemaphoreGive(uart_port_mutex);
  return err;
}
anedya_err_t anedya_ext_set_fun_mode(anedya_client_t *client, int fun, int rst,
                                     int timeout) {
  if (UART_PORT_NUMBER == -1) {
    ESP_LOGE(TAG, "Invalid port number");
    return ANEDYA_EXT_ERR;
...
// TODO: Implement dynamic allocation
  ...
  if (rst == 1) {
    err = _anedya_ext_send_AT_command(AT_cmd, MODEM_RESP_WAIT, NULL, 0, "RDY",
                                      timeout);
    _anedya_ext_send_AT_command("ATE0\r\n", MODEM_CMD_EX_ASAP, NULL, 0, NULL,
                                2000);
  } else {
    err = _anedya_ext_send_AT_command(AT_cmd, MODEM_CMD_EX_ASAP, NULL, 0, NULL,
                                      timeout);
  }
  xSemaphoreGive(uart_port_mutex);
  return err;
...
anedya_err_t anedya_ext_connectivity_check(anedya_client_t *client,
                                           int timeout) {
  if (UART_PORT_NUMBER == -1) {
    ESP_LOGE(TAG, "Invalid port number");
    return ANEDYA_EXT_ERR;
  }
  if (xSemaphoreTake(uart_port_mutex, portMAX_DELAY) != pdTRUE) {
    ESP_LOGE(TAG, "Failed to take uart_port_mutex!");
    return ANEDYA_ERR_EXT_TIMEOUT;
  }
...
anedya_err_t anedya_ext_network_reg_status(anedya_client_t *client, int *stat,
                                           int timeout) {
  if (UART_PORT_NUMBER == -1) {
    ESP_LOGE(TAG, "Invalid port number");
    return ANEDYA_EXT_ERR;
...
// TODO: Implement dynamic allocation
  ...
  anedya_err_t err = ANEDYA_EXT_ERR;
  err =
      _anedya_ext_send_AT_command("AT+CEREG?\r\n", MODEM_RESP_WAIT, s_response,
                                  sizeof(s_response), "+CEREG:", timeout);
  xSemaphoreGive(uart_port_mutex);
...
anedya_err_t anedya_ext_network_operator(anedya_client_t *client, int *mode,
                                         int timeout) {
  if (UART_PORT_NUMBER == -1) {
    ESP_LOGE(TAG, "Invalid port number");
    return ANEDYA_EXT_ERR;
...
// TODO: Implement dynamic allocation
  ...
  err = _anedya_ext_send_AT_command("AT+COPS?\r\n", MODEM_RESP_WAIT, s_response,
                                    sizeof(s_response), "+COPS:", timeout);
  xSemaphoreGive(uart_port_mutex);
...
anedya_err_t anedya_ext_signal_quality(anedya_client_t *client, int *rssi,
                                       int *ber, int timeout) {
  if (UART_PORT_NUMBER == -1) {
    ESP_LOGE(TAG, "Invalid port number");
    return ANEDYA_EXT_ERR;
...
anedya_err_t anedya_ext_pdp_context_status(anedya_client_t *client,
                                           char *pdp_context_out, int timeout) {
  if (UART_PORT_NUMBER == -1) {
    ESP_LOGE(TAG, "Invalid port number");
    return ANEDYA_EXT_ERR;
  }
  ...
  err = _anedya_ext_send_AT_command("AT+CGDCONT?\r\n", MODEM_RESP_WAIT,
                                    pdp_context_out, 200, "+CGDCONT:", timeout);
  xSemaphoreGive(uart_port_mutex);
  return err;
}
anedya_err_t anedya_ext_activate_pdp_context(anedya_client_t *client, int cid,
                                             int timeout) {
  if (UART_PORT_NUMBER == -1) {
    ESP_LOGE(TAG, "Invalid port number");
    return ANEDYA_EXT_ERR;
...
// TODO: Implement dynamic allocation
  ...
  err = _anedya_ext_send_AT_command(AT_cmd, MODEM_RESP_WAIT, NULL, 0, "OK",
                                    timeout);
  xSemaphoreGive(uart_port_mutex);
  return err;
}
anedya_err_t anedya_ext_deactivate_pdp_context(anedya_client_t *client, int cid,
                                               int timeout) {
  if (UART_PORT_NUMBER == -1) {
    ESP_LOGE(TAG, "Invalid port number");
    return ANEDYA_EXT_ERR;
...
anedya_err_t anedya_ext_read_pdp_context(anedya_client_t *client,
                                         char *pdp_context_out, int timeout) {
  if (UART_PORT_NUMBER == -1) {
    ESP_LOGE(TAG, "Invalid port number");
    return ANEDYA_EXT_ERR;
  ...
  err = _anedya_ext_send_AT_command("AT+QIACT?\r\n", MODEM_RESP_WAIT,
                                    pdp_context_out, 200, "+QIACT:", timeout);
  xSemaphoreGive(uart_port_mutex);
  return err;
}

anedya_err_t anedya_ext_net_check(anedya_client_t *client, char *url,
                                  int url_len, int timeout) {
  if (url_len > 100) {
  ...
  _anedya_ext_send_AT_command("AT+QSSLCFG=\"sni\",3,1\r\n", MODEM_RESP_WAIT,
                              NULL, 0, "OK", 5000);
  _anedya_ext_send_AT_command(
      "AT+QSSLCFG=\"cacert\",3,\"UFS:anedya_tls_root_ca.pem\"\r\n",
      MODEM_RESP_WAIT, NULL, 0, "OK", 5000);
#ifdef ANEDYA_ENABLE_STATIC_ALLOCATION
  char AT_cmd[100] = {0};
...
// TODO: Implement dynamic allocation
  ...
  if (_anedya_ext_send_AT_command(AT_cmd, MODEM_RESP_WAIT, NULL, 0, "CONNECT",
                                  80000) != ANEDYA_OK) {
    xSemaphoreGive(uart_port_mutex);
    return ANEDYA_EXT_ERR;
  }
  ...
  if (_anedya_ext_send_AT_command(AT_cmd, MODEM_RESP_WAIT, cmd_response,
                                  sizeof(cmd_response),
                                  "+QHTTPGET:", timeout + 1000) != ANEDYA_OK) {
    xSemaphoreGive(uart_port_mutex);
    return ANEDYA_EXT_ERR;
  }
  xSemaphoreGive(uart_port_mutex);
...
anedya_err_t anedya_ext_set_apn(anedya_client_t *client, int cid,
                                const char *ip_ver, const char *apn,
...
anedya_err_t anedya_ext_get_modem_time(anedya_client_t *client, int mode,
                                       char *output_dateTime) {
  if (UART_PORT_NUMBER == -1) {
    ESP_LOGE(TAG, "Invalid port number");
    return ANEDYA_EXT_ERR;
  }
...
// TODO: Implement dynamic allocation
  ...
  anedya_err_t err = _anedya_ext_send_AT_command(AT_cmd, MODEM_RESP_WAIT, temp,
                                                 sizeof(temp), "+QLTS:", 10000);
  if (err == ANEDYA_OK) {
...
anedya_mqtt_client_handle_t _anedya_interface_mqtt_init(anedya_client_t *parent,
                                                        char *broker,
                                                        const char *devid,
                                                        const char *secret) {
...
// TODO: Implement dynamic allocation
  ...
  err = _anedya_ext_send_AT_command(
      "AT+QFOPEN=\"anedya_tls_root_ca.pem\",1\r\n", MODEM_RESP_WAIT, response,
      sizeof(response), "+QFOPEN:", 2000);
  if (err != ANEDYA_OK) {
  ...
  err = _anedya_ext_send_AT_command(AT_cmd, MODEM_RESP_WAIT, response,
                                    sizeof(response), "CONNECT", 10000);
  if (err != ANEDYA_OK) {
  ...
  err =
      _anedya_ext_send_AT_command(AT_cmd, MODEM_RESP_WAIT, NULL, 0, "OK", 5000);

  // Config broker credentials
  // NOTE: ssl context id is 2 for the mqtt client, will set it 3 for the http
  // client
...
anedya_err_t
anedya_interface_mqtt_connect(anedya_mqtt_client_handle_t anclient) {
  if (UART_PORT_NUMBER == -1) {
    ESP_LOGE(TAG, "Invalid port number");
    return ANEDYA_EXT_ERR;
  }
...
// TODO: Implement dynamic allocation
  ...
  if (matched) {
    if (result_code == 0) {
      ...
      ESP_LOGE(TAG, "Failed to open MQTT connection, response: %s",
               (char *)response);
      xSemaphoreGive(uart_port_mutex);
      return ANEDYA_EXT_ERR;
    }
...
anedya_err_t
anedya_interface_mqtt_disconnect(anedya_mqtt_client_handle_t anclient) {

  return ANEDYA_OK;
}

anedya_err_t
anedya_interface_mqtt_destroy(anedya_mqtt_client_handle_t anclient) {
...
anedya_err_t
anedya_interface_mqtt_subscribe(anedya_mqtt_client_handle_t anclient,
                                char *topic, int topilc_len, int qos) {
  if (UART_PORT_NUMBER == -1) {
    ESP_LOGE(TAG, "Invalid port number");
    return ANEDYA_EXT_ERR;
  }
...
// TODO: Implement dynamic allocation
  ...
  anedya_err_t err = _anedya_ext_send_AT_command(
      AT_cmd, MODEM_RESP_WAIT, response, sizeof(response), "+QMTSUB:", 5000);
  if (err == ANEDYA_OK) {
    if (debug_level > 3)
...
anedya_err_t
anedya_interface_mqtt_unsubscribe(anedya_mqtt_client_handle_t anclient,
                                  char *topic, int topic_len) {

  return ANEDYA_OK;
}

anedya_err_t anedya_interface_mqtt_publish(anedya_mqtt_client_handle_t anclient,
                                           char *topic, int topic_len,
  ...
  if (xSemaphoreTake(uart_port_mutex, 60000 / portTICK_PERIOD_MS) == pdTRUE) {
    ...
    snprintf(AT_cmd, sizeof(AT_cmd), "AT+QMTPUBEX=0,1,%d,%d,\"%s\",%d\r\n", qos,
             retain, (char *)topic, payload_len);
    anedya_err_t err = _anedya_ext_send_AT_command(AT_cmd, MODEM_RESP_WAIT,
                                                   NULL, 0, ">", 10000);
    if (err == ANEDYA_OK) {

      err = _anedya_ext_send_AT_command((char *)payload, MODEM_RESP_WAIT, NULL,
                                        0, "}", 5000);
    }
...
anedya_err_t anedya_set_message_callback(anedya_mqtt_client_handle_t anclient,
                                         anedya_client_t *client) {
  return ANEDYA_OK;
}
#endif // end of ANEDYA_CONNECTION_METHOD_MQTT

anedya_err_t anedya_ext_http_get_range_request(anedya_client_t *client,
                                               anedya_ext_net_reader_t *reader,
                                               char *url, int url_len,
                                               int starting_position,
                                               int readlen, int timeout) {
  if (UART_PORT_NUMBER == -1) {
    ESP_LOGE(TAG, "Invalid port number");
    return ANEDYA_EXT_ERR;
  }
...
// TODO: Implement dynamic allocation
  ...
  if (_anedya_ext_send_AT_command(AT_cmd, MODEM_RESP_WAIT, NULL, 0,
                                  "+QHTTPGET:", 80000) != ANEDYA_OK) {
    xSemaphoreGive(uart_port_mutex);
    xEventGroupSetBits(ModemEvents, MODEM_EVENT_OTA_NOT_IN_PROGRESS);
    return ANEDYA_EXT_ERR;
  }
...
// TODO: Implement dynamic allocation
  ...
  while (!headers_done &&
         (xTaskGetTickCount() - start < pdMS_TO_TICKS(timeout))) {
...
size_t anedya_ext_ota_read_next(anedya_client_t *client,
                                anedya_ext_net_reader_t *reader, int read_len,
  ...
  size_t read = uart_read_bytes(UART_PORT_NUMBER, output, read_len,
...
anedya_err_t anedya_ext_ota_reader_close(anedya_client_t *client,
                                         anedya_ext_net_reader_t *reader) {
  if (UART_PORT_NUMBER == -1) {
    ESP_LOGE(TAG, "Invalid port number");
    return ANEDYA_EXT_ERR;
