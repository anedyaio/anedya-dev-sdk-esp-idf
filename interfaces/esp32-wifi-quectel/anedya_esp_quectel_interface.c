...
// Network Interface is Wifi- Quectel
...
anedya_err_t _anedya_quectel_interface_init(anedya_client_t *client)
{
    ...
    uart_driver_install(ext_config->uart_port_num, ANEDYA_RX_BUFFER_SIZE, ANEDYA_TX_BUFFER_SIZE, ANEDYA_RX_BUFFER_SIZE / 120, &ext_config->uart_queue_handle, 0);
    ...
    // Create the task
    if (xTaskCreate(_uart_event_task, "uart_event_task", 4 * 1024, client, 1, NULL) != pdPASS)
    {
    ...
    if (xTaskCreate(_anedya_ext_mqtt_event_task, "MQTT_EVENT_TASK", 4 * 1024, client, 2, NULL) != pdPASS)
...
anedya_err_t anedya_quectel_interface_mqtt_disconnect(anedya_mqtt_client_handle_t anclient)
{
    ...
    {
        anedya_err_t err = _anedya_ext_send_AT_command("AT+QMTDISC=0\r\n", MODEM_CMD_EX_ASAP, NULL, NULL, 2000); // disconnect from MQTT broker
        xSemaphoreGive(uart_port_mutex);
        return err;
    }
    else
