...
static const char *TAG = "ANEDYA_MQTT";
...
static void anedya_espi_mqtt_event_handler(void *handler_args,
  ...
  case MQTT_EVENT_DATA:
    ...
    cl->_message_handler(cl, event->topic, event->topic_len, event->data,
                         event->data_len);
    break;
...
anedya_mqtt_client_handle_t _anedya_interface_mqtt_init(anedya_client_t *parent,
  ...
  // ESP_LOGI("ANEDYA_ESPI", "Connecting to MQTT secret: %s  Length:%d", secret,
  // parent->config->connection_key_len);
  client = esp_mqtt_client_init(&mqtt_cfg);
  esp_mqtt_client_register_event(
      client, ESP_EVENT_ANY_ID, anedya_espi_mqtt_event_handler, (void *)parent);
...
anedya_err_t anedya_interface_mqtt_publish(anedya_mqtt_client_handle_t anclient,
  ...
  esp_mqtt_client_handle_t *c = (esp_mqtt_client_handle_t *)anclient;
  int id =
      esp_mqtt_client_publish(*c, topic, payload, payload_len, qos, retain);
  if (id == -1) {
    return ANEDYA_ERR;
  }
  if (id == -2) {
